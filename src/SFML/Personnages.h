...
class Personnage {
	public:
		...
		int combat(Personnage* p); //Affecte les points de degats aux personnage en fonction de leurs positions en appelant degat
		int degat(const int &atkA, bool gauche = true); //Inflige les degats aux personnages
		void pvUpdate(); //Mets a jour les PV des personnages dans la barre de vie
		...
		int setSprites(AnimPerso* s_template, sf::Texture *barre_pv, sf::Texture *texturePV); //Fixe les sprites pour l'initialisation
...
class Hero : public Personnage {
	public:
		...
		Hero(std::string sname); //Constructeur du hero avec le nom en parametres
		~Hero(); //Destructeur du hero
...
class Ennemi : public Personnage {
	public:
		...
		sf::Vector2f update(int h, int b, int g, int d, Personnage* hero); // """" IA de l'ennemi """"
		~Ennemi(); //Destructeur de l'ennemi
};
